...
object DwdUnifyThirdlyBM {
  val logger = LoggerFactory.getLogger(DwdUnifyThirdlyBM.getClass)
  ...
  def runData(startTimeP: String, endTimeP: String, isDeleteData: Boolean, conn: Connection): Unit = {
    ...
    val sql_dwd_bm_thirdly_turnover =
      s"""
         ...
         |from
         |(
         |select  * from ods_bm_platform_orders
         |where game_start_time>='$startTime' and  game_start_time<='$endTime'
         |and  status=1
         | and  thirdly_code not in ('IM','YB','YABO')
         |) t_b
         |join (select  *  from  doris_dt.dwd_users  where site_code='BM' )  t_u  on  t_b.site_code=t_u.site_code and    t_b.user_id=t_u.id
    ...
    val sql_dwd_bm_thirdly_turnover_transactions =
      s"""
         ...
         |WHEN 20012 THEN 'MG'
         |WHEN 20013 THEN 'BL'
         |WHEN 20014 THEN 'yabo'
         |WHEN 20015 THEN 'BG'
         |WHEN 20016 THEN 'PG'
         |WHEN 20017 THEN 'CX'
         |WHEN 20018 THEN 'KY'
         |WHEN 20023 THEN 'TCG'
         |WHEN 20026 THEN 'yabo'
         |WHEN 20022 THEN 'BG'
         |WHEN 20024 THEN 'PG'
         |WHEN 20027 THEN 'KY'
         ...
         |from
         |(select *  from  doris_dt.ods_bm_transactions  where  (created_at>='$startTime' and  created_at<='$endTime'))t_f
         |join  (select * from dwd_thirdly_transaction_types ) t_t   on  t_f.type_id=t_t.type_code and   t_f.site_code=t_t.site_code
         |join   (select *  from doris_dt.dwd_users where  site_code='BM')  t_u  on t_f.site_code=t_u.site_code and  t_f.user_id=t_u.id
         |""".stripMargin
    val sql_del_dwd_bm_thirdly_transactions = s"delete from  dwd_thirdly_transactions   where     site_code='BM' and (created_at>='$startTime' and  created_at<='$endTime')"
    val sql_del_dwd_bm_thirdly_turnover = s"delete from  dwd_thirdly_turnover   where     site_code='BM' and (data_time>='$startTime' and  data_time<='$endTime')"

    // 删除数据
    val start = System.currentTimeMillis()
    JdbcUtils.execute(conn, "use doris_thirdly", "use doris_thirdly")
    if (isDeleteData) {
      JdbcUtils.executeSiteDeletePartitionMonth(startTime,endTime,"",conn, "sql_del_dwd_bm_thirdly_transactions", sql_del_dwd_bm_thirdly_transactions)
      JdbcUtils.executeSiteDeletePartitionMonth(startTime,endTime,"",conn, "sql_del_dwd_bm_thirdly_turnover", sql_del_dwd_bm_thirdly_turnover)
    }
    JdbcUtils.execute(conn, "sql_dwd_bm_thirdly_turnover", sql_dwd_bm_thirdly_turnover)
    JdbcUtils.execute(conn, "sql_dwd_bm_thirdly_turnover_transactions", sql_dwd_bm_thirdly_turnover_transactions)
//    val map: Map[String, String] = Map(
//      "sql_dwd_bm_thirdly_turnover" -> sql_dwd_bm_thirdly_turnover
//      , "sql_dwd_bm_thirdly_turnover_transactions" -> sql_dwd_bm_thirdly_turnover_transactions
//    )
