...
object OdsSynThirdlyBM {
  ...
  def runData(startTimeP: String, endTimeP: String, isDeleteData: Boolean, conn: Connection): Unit = {
    ...
    val sql_ods_bm_joker_platform_orders =
      s"""
         |insert into  ods_bm_platform_orders
         |select  game_start_time,'BM' site_code,'JK' thirdly_code,user_id,id,order_no,user_name,platform_username,game_name,game_type,amount,actual_amount,prize,rate,status,status_transfer,status_commission,client,updated_at,created_at,0 order_type,0 transfer_amount,detail_id
         |from  syn_bm_joker_platform_orders
         |where (game_start_time>='$startTime' and  game_start_time<='$endTime')
         |""".stripMargin
    ...
    JdbcUtils.execute(conn, "sql_ods_bm_joker_platform_orders", sql_ods_bm_joker_platform_orders)
