 ...
 */
object AppDayKpi {
  val logger = LoggerFactory.getLogger(AppDayKpi.getClass)
  ...
  def runData(siteCode: String, startTimeP: String, endTimeP: String, isDeleteData: Boolean, conn: Connection): Unit = {
    ...
    val app_day_user_base_kpi =
      s"""
         ...
         |(
         |select data_date, site_code, user_id,user_chain_names, is_agent, is_tester, username, parent_id,  parent_username ,user_level,is_vip,  is_joint,user_created_at from app_day_user_base_kpi  where    data_date>='$startDay' and  data_date<='$endDayBase'
         |union
         |select data_date, site_code, user_id,user_chain_names, is_agent, is_tester, username, parent_id,  parent_username ,user_level,is_vip,  is_joint,user_created_at from dws_day_site_user_bank  where    data_date>='$startDay' and  data_date<='$endDayBase'
         |union
         |select data_date, site_code, user_id,user_chain_names, is_agent, is_tester, username, parent_id,  parent_username ,user_level,is_vip,  is_joint,user_created_at from dws_day_site_user_transactions  where    data_date>='$startDay' and  data_date<='$endDayBase'
         |union
         |select data_date, site_code, user_id,user_chain_names, is_agent, is_tester, username, parent_id,  parent_username ,user_level,is_vip,  is_joint,user_created_at from dws_day_site_user_logins  where    data_date>='$startDay' and  data_date<='$endDayBase'
         |union
         |select data_date, site_code, user_id,user_chain_names, is_agent, is_tester, username, parent_id,  parent_username ,user_level,is_vip,  is_joint,user_created_at from dws_day_site_user_register  where    data_date>='$startDay' and  data_date<='$endDayBase'
         |) t  group  by data_date, site_code, user_id
         |) t
         |left join (select * from dws_day_site_user_bank where    data_date>='$startDay' and  data_date<='$endDayBase' ) t_b on t.data_date=t_b.data_date and t.site_code=t_b.site_code and t.user_id=t_b.user_id
         |left join (select * from dws_day_site_user_logins where    data_date>='$startDay' and  data_date<='$endDayBase' ) t_l on t.data_date=t_l.data_date and t.site_code=t_l.site_code and t.user_id=t_l.user_id
         |left join (select * from dws_day_site_user_register where    data_date>='$startDay' and  data_date<='$endDayBase' ) t_r on t.data_date=t_r.data_date and t.site_code=t_r.site_code and t.user_id=t_r.user_id
         |left join (select * from dws_day_site_user_transactions where    data_date>='$startDay' and  data_date<='$endDayBase' ) t_t on t.data_date=t_t.data_date and t.site_code=t_t.site_code and t.user_id=t_t.user_id
         |""".stripMargin

    val sql_app_day_user_base_total_kpi =
      s"""
         |insert into  app_day_user_base_total_kpi
         |select
         |t.data_date
         |,t.site_code
         |,t.user_id
         ...
         |from
         |(
         |select data_date, site_code, user_id,max(user_chain_names)  user_chain_names,max(is_agent)  is_agent,max(is_tester)  is_tester,max(username) username, max(parent_id) parent_id, max(parent_username) parent_username , max(user_level) user_level, max(is_vip) is_vip, max(is_joint) is_joint, max(user_created_at) user_created_at,max(update_date) update_date from app_day_user_base_kpi  where    data_date>='$startDay' and  data_date<='$endDayBase'   group by data_date, site_code, user_id
         |union
         ...
         |from
         |(select *  from  doris_thirdly.app_third_day_user_kpi where    data_date>='$startDay' and  data_date<='$endDayBase' )   t
         |join  dwd_users  u  on  t.site_code=u.site_code   and  t.user_id=u.id
         |  group by t.data_date, t.site_code, t.user_id
         |) t
         |left join (select * from app_day_user_base_kpi where    data_date>='$startDay' and  data_date<='$endDayBase' ) t_b on t.data_date=t_b.data_date and t.site_code=t_b.site_code and t.user_id=t_b.user_id
         |left join (select * from doris_thirdly.app_third_day_user_kpi where    data_date>='$startDay' and  data_date<='$endDayBase' ) t_t on t.data_date=t_t.data_date and t.site_code=t_t.site_code and t.user_id=t_t.user_id
         |""".stripMargin

    val sql_app_day_user_lost_kpi =
      s"""
         |insert into app_day_user_lost_kpi
         ...
         |,sum(if(datediff(t2.data_date,t.data_date) <=0 and t.register_user_count>0,ifnull(t2.third_turnover_valid_amount,0),0)) third_turnover_valid_up_0
         |,sum(if(datediff(t2.data_date,t.data_date) <=1 and t.register_user_count>0,ifnull(t2.third_turnover_valid_amount,0),0)) third_turnover_valid_up_1
         |,sum(if(datediff(t2.data_date,t.data_date) <=7 and t.register_user_count>0,ifnull(t2.third_turnover_valid_amount,0),0)) third_turnover_valid_up_7
         |,sum(if(datediff(t2.data_date,t.data_date) <=15 and t.register_user_count>0,ifnull(t2.third_turnover_valid_amount,0),0)) third_turnover_valid_up_15
         |,sum(if(datediff(t2.data_date,t.data_date) <=30 and t.register_user_count>0,ifnull(t2.third_turnover_valid_amount,0),0)) third_turnover_valid_up_30
         ...
         |from
         |(
         |select  *  from app_day_user_base_total_kpi  where    (data_date >='$startDay'  and   data_date <= '$endDay')
         ...
         |) t
         |join
         |( select  *  from app_day_user_base_total_kpi
         |where    (data_date >='$startDay'  and   data_date <=date_add('$endDay',7))
         |and (
         |(
    ...
    val sql_app_day_user_up_kpi =
      s"""
         ...
         |,update_date
         |from app_day_user_base_total_kpi
         |where    data_date>='2021-01-01'
         |)  t
         |where     (data_date>='$startDay' and  data_date<='$endDay')
         |""".stripMargin

    val sql_app_day_user_kpi =
      s"""
         |insert into  app_day_user_kpi
         |select
         |t.data_date
         |,t.site_code
         |,t.user_id
         ...
         |from
         |(
         |select data_date, site_code, user_id,max(user_chain_names)  user_chain_names,max(is_agent)  is_agent,max(is_tester)  is_tester,max(username) username, max(parent_id) parent_id, max(parent_username) parent_username , max(user_level) user_level, max(is_vip) is_vip, max(is_joint) is_joint, max(user_created_at) user_created_at from app_day_user_base_total_kpi  where    data_date>='$startDay' and  data_date<='$endDay'   group by data_date, site_code, user_id
         |) t
         |left join (select * from app_day_user_base_total_kpi where    data_date>='$startDay' and  data_date<='$endDay' ) t_b on t.data_date=t_b.data_date and t.site_code=t_b.site_code and t.user_id=t_b.user_id
         |left join (select * from app_day_user_lost_kpi where    data_date>='$startDay' and  data_date<='$endDay' ) t_l on t.data_date=t_l.data_date and t.site_code=t_l.site_code and t.user_id=t_l.user_id
         |left join (select * from app_day_user_up_kpi where    data_date>='$startDay' and  data_date<='$endDay' ) t_p on t.data_date=t_p.data_date and t.site_code=t_p.site_code and t.user_id=t_p.user_id
         |""".stripMargin


    // 日报-平台盈亏
    val sql_app_day_site_kpi =
      s"""
         |insert  into  app_day_site_kpi
         |select
         |data_date
         |,site_code
         ...
         |,if(t.data_date>='$sysDate','$endTime',concat(t.data_date, ' 23:59:59'))  update_date
         |from app_day_user_kpi t
         |where    (data_date>='$startDay' and   data_date<='$endDay')   and  is_tester=0
         |group   by   data_date,site_code
         |""".stripMargin

    val sql_del_app_day_user_base_kpi = s"delete from  app_day_user_base_kpi  where    (data_date>='$startDay' and  data_date<='$endDay')"
    val sql_del_app_day_user_up_kpi = s"delete from  app_day_user_up_kpi  where    (data_date>='$startDay' and  data_date<='$endDay')"
    val sql_del_app_day_user_lost_kpi = s"delete from  app_day_user_lost_kpi  where    (data_date>='$startDay' and  data_date<='$endDay')"
    val sql_del_app_day_user_base_total_kpi = s"delete from  app_day_user_base_total_kpi  where    (data_date>='$startDay' and  data_date<='$endDay')"

    val sql_del_app_day_user_kpi = s"delete from  app_day_user_kpi  where    (data_date>='$startDay' and  data_date<='$endDay')"
    val sql_del_app_day_site_kpi = s"delete from  app_day_site_kpi  where    (data_date>='$startDay' and  data_date<='$endDay')"

    val start = System.currentTimeMillis()
    // 删除数据
    JdbcUtils.executeSite(siteCode, conn, "use doris_dt", "use doris_dt")
    if (isDeleteData) {
      JdbcUtils.executeSiteDeletePartitionMonth(startDay,endDay,siteCode, conn, "sql_del_app_day_user_base_kpi", sql_del_app_day_user_base_kpi)
      JdbcUtils.executeSiteDeletePartitionMonth(startDay,endDay,siteCode, conn, "sql_del_app_day_user_up_kpi", sql_del_app_day_user_up_kpi)
      JdbcUtils.executeSiteDeletePartitionMonth(startDay,endDay,siteCode, conn, "sql_del_app_day_user_lost_kpi", sql_del_app_day_user_lost_kpi)
      JdbcUtils.executeSiteDeletePartitionMonth(startDay,endDay,siteCode, conn, "sql_del_app_day_user_base_total_kpi", sql_del_app_day_user_base_total_kpi)

      JdbcUtils.executeSiteDeletePartitionMonth(startDay,endDay,siteCode, conn, "sql_del_app_day_user_kpi", sql_del_app_day_user_kpi)
      JdbcUtils.executeSiteDelete(siteCode, conn, "sql_del_app_day_site_kpi", sql_del_app_day_site_kpi)
    }
    // 日报-用户盈亏报表
    JdbcUtils.executeSite(siteCode, conn, "app_day_user_base_kpi", app_day_user_base_kpi)
    JdbcUtils.executeSite(siteCode, conn, "sql_app_day_user_base_total_kpi", sql_app_day_user_base_total_kpi)
    JdbcUtils.executeSite(siteCode, conn, "sql_app_day_user_lost_kpi", sql_app_day_user_lost_kpi)
    if (isDeleteData) {
      // 日报-用户盈亏报表
      // JdbcUtils.executeSite(siteCode, conn, "sql_app_day_user_up_kpi", sql_app_day_user_up_kpi)
    }
    JdbcUtils.executeSite(siteCode, conn, "sql_app_day_user_kpi", sql_app_day_user_kpi)
    // 日报-平台盈亏报表
    JdbcUtils.executeSite(siteCode, conn, "sql_app_day_site_kpi", sql_app_day_site_kpi)
  ...
  def runGroupData(siteCode: String, startTimeP: String, endTimeP: String, isDeleteData: Boolean, conn: Connection): Unit = {
    ...
    val sql_app_day_group_user_zipper_kpi_base =
      s"""
         |select
         |'$endDay' as data_date
         |,t.site_code
         ...
         |join (select *  from dwd_users where    is_tester=0 and is_agent=1 ) t_u on  t_g.site_code=t_u.site_code  and  t_g.group_username =t_u.username
         |""".stripMargin

    val sql_app_group_user_count_kpi_base =
      s"""
         |select
         |site_code
         ...
         |,now() as update_at
         |from   doris_dt.dwd_users
         |where      (!null_or_empty(split_part(user_chain_names,'/',group_level_num)))  and  is_tester=0
         |group  by  site_code ,split_part(user_chain_names,'/',group_level_num)
         |""".stripMargin

    val sql_app_day_group_kpi_base =
      s"""
         |select
         |t.data_date
         |,t.site_code
         ...
         |) t
         |join (
         |select *  from  app_day_group_user_zipper_kpi where     (data_date>='$startDay' and   data_date<='$endDay')  and group_level=  (group_level_num-2)
    ...
    val sql_del_app_day_group_kpi = s"delete from  app_day_group_kpi  where    (data_date>='$startDay' and  data_date<='$endDay')"
    val start = System.currentTimeMillis()
    // 删除数据
    JdbcUtils.executeSite(siteCode, conn, "use doris_dt", "use doris_dt")
    if (isDeleteData) {
      JdbcUtils.executeSiteDeletePartitionMonth(startDay,endDay,siteCode, conn, "sql_del_app_day_group_kpi", sql_del_app_day_group_kpi)
    }
    // 团队用户数
    val max_group_level_num = JdbcUtils.queryCount(siteCode, conn, "sql_app_day_group_kpi_max", s"select max(user_level) max_user_level from  app_day_user_base_kpi  where    (data_date>='$startDay' and   data_date<='$endDay') ")
    if (DateUtils.getEndDate.substring(0, 10).equals(endDay)) {
      for (groupLevelNum <- 2 to max_group_level_num + 3) {
        JdbcUtils.executeSite(siteCode, conn, "sql_app_group_user_count_kpi_" + (groupLevelNum - 2), AppGroupUtils.concatSqlOnce("app_group_user_count_kpi", sql_app_group_user_count_kpi_base, groupLevelNum))
        Thread.sleep(5000);
      }
    }
    ...
    for (day <- 0 to days) {
      ...
      val sql_app_day_group_user_zipper_kpi_base_day = sql_app_day_group_user_zipper_kpi_base
        .replace(startDay, runDay)
        .replace(endDay, runDay)
        .replace(startTime, runDayStart)
        .replace(endTime, runDayEnd)
      val max_group_level_num_day = JdbcUtils.queryCount(siteCode, conn, "sql_app_day_group_kpi_max", s"select max(user_level) max_user_level from  app_day_user_base_kpi  where    (data_date>='$runDayStart' and   data_date<='$runDayEnd') ")
      for (groupLevelNum <- 2 to max_group_level_num_day + 3) {
        JdbcUtils.executeSite(siteCode, conn, "sql_app_day_group_user_zipper_kpi_" + runDay + "_" + (groupLevelNum - 2), AppGroupUtils.concatSqlOnce("app_day_group_user_zipper_kpi", sql_app_day_group_user_zipper_kpi_base_day, groupLevelNum))
        Thread.sleep(5000);
      }
    }
    // 日报-团队盈亏报表
    for (groupLevelNum <- 2 to max_group_level_num + 3) {
      Thread.sleep(5000);
      JdbcUtils.executeSite(siteCode, conn, "sql_app_day_group_kpi_" + (groupLevelNum - 2), AppGroupUtils.concatSqlOnce("app_day_group_kpi", sql_app_day_group_kpi_base, groupLevelNum))
    }
    val end = System.currentTimeMillis()
    logger.info("AppDayUserKpi 累计耗时(毫秒):" + (end - start))
  }
  ...
  def runTurnoverData(siteCode: String, startTimeP: String, endTimeP: String, isDeleteData: Boolean, conn: Connection): Unit = {
    ...
    logger.warn(s" startTime : '$startDay'  , endTime '$endDay', isDeleteData '$isDeleteData'")
    // 日报-用户-彩种-玩法-期数盈亏
    val sql_app_day_user_lottery_issue_turnover_kpi =
      s"""
         |insert into app_day_user_lottery_issue_turnover_kpi
         |select
         |data_date
         |,site_code
         |,user_id
         ...
         |,if(t.data_date>='$sysDate','$endTime',concat(t.data_date, ' 23:59:59'))  update_date
         |from
         |dws_day_site_user_lottery_issue_turnover_transactions t
         |where     (data_date>='$startDay' and   data_date<='$endDay')
         |""".stripMargin

    //注意: 一期的投注会跨天
    val sql_app_day_lottery_issue_kpi =
      s"""
         |insert  into app_day_lottery_issue_kpi
         |select
         |t_1.issue_date  as data_date
         |,t_1.site_code
         |,t_2.series_code
         |,t_1.lottery_code
         |,t_1.issue
         ...
         |from
         |(
         | select  distinct site_code,lottery_code, issue,issue_date,issue_web  from  app_day_user_lottery_issue_turnover_kpi
         |  where      (issue_date>='$startTime' and   issue_date<='$endTime')
         |) t_1
         |join  (
         |select *  from  app_day_user_lottery_issue_turnover_kpi
         |where     (data_date>=date_add('$startDay',-5) and   data_date<=date_add('$endDay',4))   and  is_tester=0
         |) t_2 on  t_1.site_code=t_2.site_code and   t_1.lottery_code=t_2.lottery_code and  t_1.issue=t_2.issue
         |group  by  t_1.issue_date,t_1.site_code,t_2.series_code,t_1.lottery_code,t_1.issue
         |""".stripMargin

    // 日报-用户-彩种-投注方式盈亏
    val sql_app_day_user_lottery_turnover_kpi =
      s"""
         |insert  into  app_day_user_lottery_turnover_kpi
         |select
         |data_date
         |,site_code
         |,user_id
         ...
         |,max(update_date) update_date
         |from
         |app_day_user_lottery_issue_turnover_kpi
         |where     data_date>='$startDay' and   data_date<='$endDay'
         |group  by
         |data_date, site_code,user_id,user_chain_names,is_agent,is_tester,series_code,lottery_code,turnover_code
         |""".stripMargin
    val sql_app_day_user_lottery_kpi =
      s"""
         |insert  into  app_day_user_lottery_kpi
         |select
         |data_date
         |,site_code
         |,user_id
    ...
    val sql_app_day_lottery_turnover_kpi =
      s"""
         |insert  into  app_day_lottery_turnover_kpi
         |select
         |data_date
         |,site_code
         |,series_code
         ...
         |,max(update_date) update_date
         |from
         |app_day_user_lottery_turnover_kpi
         |where     (data_date>='$startDay' and   data_date<='$endDay') and  is_tester=0
         |group  by
         |data_date, site_code,series_code,lottery_code,turnover_code
         |""".stripMargin

    val sql_app_day_lottery_kpi =
      s"""
         |insert  into  app_day_lottery_kpi
         |select
         |data_date
         |,site_code
         |,series_code
    ...
    val sql_app_day_turnover_kpi =
      s"""
         |insert  into  app_day_turnover_kpi
         |select
         |data_date
         |,site_code
         |,turnover_code
    ...
    val sql_del_app_day_user_lottery_issue_turnover_kpi = s"delete from  app_day_user_lottery_issue_turnover_kpi  where    (data_date>='$startDay' and  data_date<='$endDay')"
    val sql_del_app_day_lottery_issue_kpi = s"delete from  app_day_lottery_issue_kpi  where    (data_date>='$startTime' and  data_date<='$endTime')"
    val sql_del_app_day_user_lottery_turnover_kpi = s"delete from  app_day_user_lottery_turnover_kpi  where    (data_date>='$startDay' and  data_date<='$endDay')"
    val sql_del_app_day_user_lottery_kpi = s"delete from  app_day_user_lottery_kpi  where    (data_date>='$startDay' and  data_date<='$endDay')"
    val sql_del_app_day_lottery_turnover_kpi = s"delete from  app_day_lottery_turnover_kpi  where    (data_date>='$startDay' and  data_date<='$endDay')"
    val sql_del_app_day_lottery_kpi = s"delete from  app_day_lottery_kpi  where    (data_date>='$startDay' and  data_date<='$endDay')"
    val sql_del_app_day_turnover_kpi = s"delete from  app_day_turnover_kpi  where    (data_date>='$startDay' and  data_date<='$endDay')"
    val start = System.currentTimeMillis()
    // 删除数据
    JdbcUtils.executeSite(siteCode, conn, "use doris_dt", "use doris_dt")
    if (isDeleteData) {
      JdbcUtils.executeSiteDeletePartitionMonth(startDay,endDay,siteCode, conn, "sql_del_app_day_user_lottery_issue_turnover_kpi", sql_del_app_day_user_lottery_issue_turnover_kpi)
      JdbcUtils.executeSiteDeletePartitionMonth(startDay,endDay,siteCode, conn, "sql_del_app_day_lottery_issue_kpi", sql_del_app_day_lottery_issue_kpi)
      JdbcUtils.executeSiteDeletePartitionMonth(startDay,endDay,siteCode, conn, "sql_del_app_day_user_lottery_turnover_kpi", sql_del_app_day_user_lottery_turnover_kpi)
      JdbcUtils.executeSiteDeletePartitionMonth(startDay,endDay,siteCode, conn, "sql_del_app_day_user_lottery_kpi", sql_del_app_day_user_lottery_kpi)
      JdbcUtils.executeSiteDeletePartitionMonth(startDay,endDay,siteCode, conn, "sql_del_app_day_lottery_turnover_kpi", sql_del_app_day_lottery_turnover_kpi)
      JdbcUtils.executeSiteDeletePartitionMonth(startDay,endDay,siteCode, conn, "sql_del_app_day_lottery_kpi", sql_del_app_day_lottery_kpi)
      JdbcUtils.executeSiteDeletePartitionMonth(startDay,endDay,siteCode, conn, "sql_del_app_day_turnover_kpi", sql_del_app_day_turnover_kpi)
    }
    //  日-用户-彩种-期数盈亏报表
    JdbcUtils.executeSite(siteCode, conn, "sql_app_day_user_lottery_issue_turnover_kpi", sql_app_day_user_lottery_issue_turnover_kpi)
    //  日-用户-彩种-投注方式盈亏报表
    JdbcUtils.executeSite(siteCode, conn, "sql_app_day_user_lottery_turnover_kpi", sql_app_day_user_lottery_turnover_kpi)
    //  日-用户-彩种盈亏报表
    JdbcUtils.executeSite(siteCode, conn, "sql_app_day_user_lottery_kpi", sql_app_day_user_lottery_kpi)
    // 日报-彩种-期数盈亏
    JdbcUtils.executeSite(siteCode, conn, "sql_app_day_lottery_issue_kpi", sql_app_day_lottery_issue_kpi)
    //  日-彩种-投注方式盈亏报表
    JdbcUtils.executeSite(siteCode, conn, "sql_app_day_lottery_turnover_kpi", sql_app_day_lottery_turnover_kpi)
    //  日-彩种盈亏报表
    JdbcUtils.executeSite(siteCode, conn, "sql_app_day_lottery_kpi", sql_app_day_lottery_kpi)
    //  日-投注方式盈亏报表
    JdbcUtils.executeSite(siteCode, conn, "sql_app_day_turnover_kpi", sql_app_day_turnover_kpi)
    val end = System.currentTimeMillis()
  ...
  def runGroupTurnoverData(siteCode: String, startTimeP: String, endTimeP: String, isDeleteData: Boolean, conn: Connection): Unit = {
    ...
    val endDay = endTime.substring(0, 10)
    logger.warn(s" startTime : '$startDay'  , endTime '$endDay', isDeleteData '$isDeleteData'")
    val sql_app_day_group_lottery_turnover_kpi_base =
      s"""
         |select
         |t.data_date
         |,t.site_code
    ...
    val sql_app_day_group_lottery_kpi_base =
      s"""
         |select
         |t.data_date
         |,t.site_code
         ...
         |from
         |(
         |select *  from  app_day_user_lottery_kpi
         |where     (data_date>='$startDay' and   data_date<='$endDay') and  is_tester=0
         |and   (!null_or_empty(split_part(user_chain_names,'/',group_level_num)))
         |)t
         |join (
    ...
    val sql_del_app_day_group_lottery_turnover_kpi = s"delete from  app_day_group_lottery_turnover_kpi  where    (data_date>='$startDay' and  data_date<='$endDay')"
    val sql_del_app_day_group_lottery_kpi = s"delete from  app_day_group_lottery_kpi  where    (data_date>='$startDay' and  data_date<='$endDay')"
    val start = System.currentTimeMillis()
    // 删除数据
    JdbcUtils.executeSite(siteCode, conn, "use doris_dt", "use doris_dt")
    if (isDeleteData) {
      JdbcUtils.executeSiteDeletePartitionMonth(startDay,endDay,siteCode, conn, "sql_del_app_day_group_lottery_turnover_kpi", sql_del_app_day_group_lottery_turnover_kpi)
      JdbcUtils.executeSiteDeletePartitionMonth(startDay,endDay,siteCode, conn, "sql_del_app_day_group_lottery_kpi", sql_del_app_day_group_lottery_kpi)
    }
    val max_group_level_num = JdbcUtils.queryCount(siteCode, conn, "sql_app_day_group_kpi_max", s"select max(user_level) max_user_level from  app_day_user_lottery_issue_turnover_kpi  where    (data_date>='$startDay' and   data_date<='$endDay') ")
    // 日报-团队盈亏报表
    for (groupLevelNum <- 2 to max_group_level_num + 3) {
      Thread.sleep(5000);
      // 日-团队-彩种-投注方式盈亏报表
      JdbcUtils.executeSite(siteCode, conn, "sql_app_day_group_lottery_turnover_kpi_" + (groupLevelNum - 2), AppGroupUtils.concatSqlOnce("app_day_group_lottery_turnover_kpi", sql_app_day_group_lottery_turnover_kpi_base, groupLevelNum))
      // 日-团队-彩种盈亏报表
      JdbcUtils.executeSite(siteCode, conn, "sql_app_day_group_lottery_kpi_" + (groupLevelNum - 2), AppGroupUtils.concatSqlOnce("app_day_group_lottery_kpi", sql_app_day_group_lottery_kpi_base, groupLevelNum))
    }
  ...
  def runTransactionData(siteCode: String, startTimeP: String, endTimeP: String, isDeleteData: Boolean, conn: Connection): Unit = {
    ...
    val sql_app_day_user_transaction_kpi =
      s"""
         |insert  into app_day_user_transaction_kpi
         ...
         |from (select  *  from  dwd_transactions where    created_at>='$startTime' and  created_at<='$endTime') t
    ...
    val sql_app_day_user_transaction_lottery_turnover_kpi =
      s"""
         |insert  into app_day_user_transaction_lottery_turnover_kpi
    ...
    val sql_app_day_transaction_kpi =
      s"""
         |insert  into app_day_transaction_kpi
    ...
    val sql_app_day_transaction_lottery_turnover_kpi =
      s"""
         |insert  into app_day_transaction_lottery_turnover_kpi
         ...
         |,max(update_date)  update_date
         |from
         |app_day_user_transaction_lottery_turnover_kpi
         |where    data_date>='$startDay' and  data_date<='$endDay' and  is_tester=0
         |group  by  data_date,site_code,type_code,series_code,lottery_code,turnover_code
         |""".stripMargin

    val sql_del_app_day_user_transaction_kpi = s"delete from  app_day_user_transaction_kpi  where    (data_date>='$startDay' and  data_date<='$endDay')"
    val sql_del_app_day_user_transaction_lottery_turnover_kpi = s"delete from  app_day_user_transaction_lottery_turnover_kpi  where    (data_date>='$startDay' and  data_date<='$endDay')"
    val sql_del_app_day_transaction_kpi = s"delete from  app_day_transaction_kpi  where    (data_date>='$startDay' and  data_date<='$endDay')"
    val sql_del_app_day_transaction_lottery_turnover_kpi = s"delete from  app_day_transaction_lottery_turnover_kpi  where    (data_date>='$startDay' and  data_date<='$endDay')"

    JdbcUtils.execute(conn, "use doris_dt", "use doris_dt")
    if (isDeleteData) {
      JdbcUtils.executeSiteDeletePartitionMonth(startDay,endDay,siteCode, conn, "sql_del_app_day_user_transaction_kpi", sql_del_app_day_user_transaction_kpi)
      JdbcUtils.executeSiteDeletePartitionMonth(startDay,endDay,siteCode, conn, "sql_del_app_day_user_transaction_lottery_turnover_kpi", sql_del_app_day_user_transaction_lottery_turnover_kpi)
      JdbcUtils.executeSiteDeletePartitionMonth(startDay,endDay,siteCode, conn, "sql_del_app_day_transaction_kpi", sql_del_app_day_transaction_kpi)
      JdbcUtils.executeSiteDeletePartitionMonth(startDay,endDay,siteCode, conn, "sql_del_app_day_transaction_lottery_turnover_kpi", sql_del_app_day_transaction_lottery_turnover_kpi)
    }
    JdbcUtils.executeSite(siteCode, conn, "sql_app_day_user_transaction_kpi", sql_app_day_user_transaction_kpi)
    JdbcUtils.executeSite(siteCode, conn, "sql_app_day_user_transaction_lottery_turnover_kpi", sql_app_day_user_transaction_lottery_turnover_kpi)
    JdbcUtils.executeSite(siteCode, conn, "sql_app_day_transaction_kpi", sql_app_day_transaction_kpi)
    JdbcUtils.executeSite(siteCode, conn, "sql_app_day_transaction_lottery_turnover_kpi", sql_app_day_transaction_lottery_turnover_kpi)
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    runData("BM", "2021-12-01 00:00:00", "2021-12-01 23:59:59", false, conn)
