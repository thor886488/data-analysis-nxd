 ...
 */
object DwsHourThirdlyKpi {
  val logger = LoggerFactory.getLogger(DwsHourThirdlyKpi.getClass)
  ...
  def runData(siteCode: String, startTimeP: String, endTimeP: String, isDeleteData: Boolean, conn: Connection): Unit = {
    ...
    val sql_dws_hour_site_thirdly_user_turnover_game =
      s"""
         |insert  into  dws_hour_site_thirdly_user_turnover_game
         |select
         |DATE_FORMAT(data_time,'%Y-%m-%d %H:00:00')
         |,site_code
         |,thirdly_code
         |,user_id
         |,max(username)
         ...
         |group by  DATE_FORMAT(data_time,'%Y-%m-%d %H:00:00'),site_code ,thirdly_code,user_id,game_code
         |""".stripMargin

    val sql_dws_hour_site_thirdly_user_turnover =
      s"""
         |insert  into  dws_hour_site_thirdly_user_turnover
         |select
         |DATE_FORMAT(data_time,'%Y-%m-%d %H:00:00')
         |,site_code
         |,thirdly_code
         ...
         |dwd_thirdly_turnover
         |where    (data_time>='$startTime' and  data_time<='$endTime') and is_cancel =0
         |group by  DATE_FORMAT(data_time,'%Y-%m-%d %H:00:00'),site_code ,thirdly_code,user_id
         |""".stripMargin

    val sql_dws_hour_site_thirdly_user_transactions =
      s"""
         |insert into  dws_hour_site_thirdly_user_transactions
         |select
         ...
         |group by  DATE_FORMAT(created_at,'%Y-%m-%d %H:00:00'),site_code ,thirdly_code,user_id
         |""".stripMargin

    val sql_del_dws_hour_site_thirdly_user_transactions = s"delete  from  dws_hour_site_thirdly_user_transactions  where     data_date>='$startTime' and data_date<='$endTime'  "
    val sql_del_dws_hour_site_thirdly_user_turnover = s"delete  from  dws_hour_site_thirdly_user_turnover  where     data_date>='$startTime' and data_date<='$endTime'  "
    val sql_del_dws_hour_site_thirdly_user_turnover_game = s"delete  from  dws_hour_site_thirdly_user_turnover_game  where     data_date>='$startTime' and data_date<='$endTime'  "
    ...
    if (isDeleteData) {
      //+8 时区报表
      JdbcUtils.executeSiteDeletePartitionMonth(startTime,endTime,siteCode, conn, "sql_del_dws_hour_site_thirdly_user_transactions", sql_del_dws_hour_site_thirdly_user_transactions)
      JdbcUtils.executeSiteDeletePartitionMonth(startTime,endTime,siteCode, conn, "sql_del_dws_hour_site_thirdly_user_turnover", sql_del_dws_hour_site_thirdly_user_turnover)
      JdbcUtils.executeSiteDeletePartitionMonth(startTime,endTime,siteCode, conn, "sql_del_dws_hour_site_thirdly_user_turnover_game", sql_del_dws_hour_site_thirdly_user_turnover_game)
    }
    //+8 时区报表
    JdbcUtils.executeSite(siteCode, conn, "sql_dws_hour_site_thirdly_user_turnover_game", sql_dws_hour_site_thirdly_user_turnover_game)
    JdbcUtils.executeSite(siteCode, conn, "sql_dws_hour_site_thirdly_user_turnover", sql_dws_hour_site_thirdly_user_turnover)
    JdbcUtils.executeSite(siteCode, conn, "sql_dws_hour_site_thirdly_user_transactions", sql_dws_hour_site_thirdly_user_transactions)

//    val map: Map[String, String] = Map(
//      "sql_dws_hour_site_thirdly_user_turnover_game" -> sql_dws_hour_site_thirdly_user_turnover_game
//      , "sql_dws_hour_site_thirdly_user_turnover" -> sql_dws_hour_site_thirdly_user_turnover
//      , "sql_dws_hour_site_thirdly_user_transactions" -> sql_dws_hour_site_thirdly_user_transactions
//    )
//    ThreadPoolUtils.executeSiteMap(siteCode, map, conn, "doris_thirdly")
  ...
  def main(args: Array[String]): Unit = {
    ...
    JdbcUtils.executeSite("BM", conn, "use doris_thirdly", "use doris_thirdly")
