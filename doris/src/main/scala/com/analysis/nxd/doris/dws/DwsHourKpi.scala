...
object DwsHourKpi {
  ...
  def runData(siteCode: String, startTimeP: String, endTimeP: String, isDeleteData: Boolean, conn: Connection): Unit = {
    ...
    val endTime: String = endTimeP

    logger.warn(s" startTime : '$startTime'  , endTime '$endTime', isDeleteData '$isDeleteData'")
    val sql_dws_hour_site_user_transactions =
      s"""
         |insert into  dws_hour_site_user_transactions
         |select
         |DATE_FORMAT(created_at,'%Y-%m-%d %H:00:00') data_date
         |,site_code
         |,user_id
         |,max(username)
         ...
         |,sum(if(paren_type_code_t='lower_agent_daily_wage',floor(abs(amount)*10000)  * pm_available /10000,0))   t_lower_agent_daily_wage_amount
         |,count(distinct if(paren_type_code_t='lower_agent_daily_wage',tran_no,null))   t_lower_agent_daily_wage_count
         ...
         |,sum(if(type_code='GM,RRHA,null,2',floor(abs(amount)*10000)  * pm_available /10000,0)) t_tran_rebates_lottery_amount
         |,count(distinct if(type_code='GM,RRHA,null,2',tran_no,null)) t_tran_rebates_lottery_count
         ...
         |,count(distinct if(paren_type_code_t in ('withdraw','withdraw_u') and is_first_withdraw=1,floor(abs(amount)*10000)/10000,0))   withdraw_first_amount
         ...
         |from
         |dwd_transactions t
         |left join (select distinct  site_code site_code_t,type_code type_code_t,paren_type_code paren_type_code_t,pm_available from dwd_transaction_types ) t_t on t.site_code=t_t.site_code_t and   t.type_code=t_t.type_code_t
         |left join
         |(
         ...
         |group by  DATE_FORMAT(created_at,'%Y-%m-%d %H:00:00'),site_code ,user_id
         |""".stripMargin

    val sql_dws_hour_site_user_register =
      s"""
         |insert into  dws_hour_site_user_register
         |select  DATE_FORMAT(created_at,'%Y-%m-%d %H:00:00') data_date,site_code,id
         |,max(username)
         |,max(is_agent)
         |,max(is_tester)
         |,max(parent_id)
         ...
         |where    (created_at>='$startTime' and  created_at<='$endTime')
         |group by  site_code ,DATE_FORMAT(created_at,'%Y-%m-%d %H:00:00'),id
         |""".stripMargin

    val sql_dws_hour_site_user_logins =
      s"""
         |insert into  dws_hour_site_user_logins
         |select  DATE_FORMAT(created_at,'%Y-%m-%d %H:00:00') data_date,site_code ,user_id
         |,max(username)
         |,max(user_chain_names)
         |,max(is_agent)
         |,max(is_tester)
         ...
         |where    (created_at>='$startTime' and  created_at<='$endTime')
         |group by  site_code ,DATE_FORMAT(created_at,'%Y-%m-%d %H:00:00'),user_id
         """.stripMargin
    val sql_dws_hour_site_user_lottery_issue_turnover_transactions =
      s"""
         |insert into  dws_hour_site_user_lottery_issue_turnover_transactions
         |select
         ...
         |and  type_code is  not  null
         |group by  DATE_FORMAT(created_at,'%Y-%m-%d %H:00:00'),site_code ,user_id,series_code,lottery_code,issue,turnover_code
         |""".stripMargin

    // 绑卡用户数
    val sql_dws_hour_site_user_bank =
      s"""
         |insert  into  dws_hour_site_user_bank
         |select  DATE_FORMAT(created_at,'%Y-%m-%d %H:00:00') data_date,site_code,user_id,username,count(distinct uuid) bank_count
         |,max(user_chain_names)
         |,max(is_agent)
         |,max(is_tester)
         |,max(parent_id)
         ...
         |from  dwd_user_bank
         |where    (created_at>='$startTime' and  created_at<='$endTime')
         |group  by  DATE_FORMAT(created_at,'%Y-%m-%d %H:00:00'),site_code,user_id,username
         |""".stripMargin

    val sql_del_dws_hour_site_user_transactions = s"delete  from  dws_hour_site_user_transactions  where     data_date>='$startTime' and data_date<='$endTime'  "
    val sql_del_dws_hour_site_user_register = s"delete  from  dws_hour_site_user_register  where     data_date>='$startTime' and data_date<='$endTime'  "
    val sql_del_dws_hour_site_user_logins = s"delete  from  dws_hour_site_user_logins  where     data_date>='$startTime' and data_date<='$endTime'  "
    val sql_del_dws_hour_site_user_lottery_issue_turnover_transactions = s"delete  from  dws_hour_site_user_lottery_issue_turnover_transactions  where     data_date>='$startTime' and data_date<='$endTime'"
    val sql_del_dws_hour_site_user_bank = s"delete  from  dws_hour_site_user_bank  where     data_date>='$startTime' and data_date<='$endTime'"

    val start = System.currentTimeMillis()
    // 删除数据
    JdbcUtils.executeSite(siteCode, conn, "use doris_dt", "use doris_dt")
    if (isDeleteData) {
      JdbcUtils.executeSiteDeletePartitionMonth(startTime, endTime, siteCode, conn, "sql_del_dws_hour_site_user_register", sql_del_dws_hour_site_user_register)
      JdbcUtils.executeSiteDeletePartitionMonth(startTime, endTime, siteCode, conn, "sql_del_dws_hour_site_user_logins", sql_del_dws_hour_site_user_logins)
      JdbcUtils.executeSiteDeletePartitionMonth(startTime, endTime, siteCode, conn, "sql_del_dws_hour_site_user_transactions", sql_del_dws_hour_site_user_transactions)
      JdbcUtils.executeSiteDeletePartitionMonth(startTime, endTime, siteCode, conn, "sql_del_dws_hour_site_user_lottery_issue_turnover_transactions", sql_del_dws_hour_site_user_lottery_issue_turnover_transactions)
      JdbcUtils.executeSiteDelete(siteCode, conn, "sql_del_dws_hour_site_user_bank", sql_del_dws_hour_site_user_bank)
    }
    JdbcUtils.executeSite(siteCode, conn, "sql_dws_hour_site_user_register", sql_dws_hour_site_user_register)
    JdbcUtils.executeSite(siteCode, conn, "sql_dws_hour_site_user_logins", sql_dws_hour_site_user_logins)
    JdbcUtils.executeSite(siteCode, conn, "sql_dws_hour_site_user_transactions", sql_dws_hour_site_user_transactions)
    JdbcUtils.executeSite(siteCode, conn, "sql_dws_hour_site_user_lottery_issue_turnover_transactions", sql_dws_hour_site_user_lottery_issue_turnover_transactions)
    JdbcUtils.executeSite(siteCode, conn, "sql_dws_hour_site_user_bank", sql_dws_hour_site_user_bank)

    //    val map: Map[String, String] = Map(
    //      "sql_dws_hour_site_user_register" -> sql_dws_hour_site_user_register
    //      , "sql_dws_hour_site_user_logins" -> sql_dws_hour_site_user_logins
    //      , "sql_dws_hour_site_user_transactions" -> sql_dws_hour_site_user_transactions
    //      , "sql_dws_hour_site_user_lottery_issue_turnover_transactions" -> sql_dws_hour_site_user_lottery_issue_turnover_transactions
    //      , "sql_dws_hour_site_user_bank" -> sql_dws_hour_site_user_bank
    //    )
    //    ThreadPoolUtils.executeSiteMap(siteCode, map, conn, "doris_dt")

    val end = System.currentTimeMillis()
    logger.info("DwsHourKpi 累计耗时(毫秒):" + (end - start))
  }

  def main(args: Array[String]): Unit = {
    val conn = JdbcUtils.getConnection()
    JdbcUtils.executeSite("all", conn, "use doris_dt", "use doris_dt")

    runData("BM", "2021-05-20 00:00:00", "2021-05-21 00:00:00", true, conn)
