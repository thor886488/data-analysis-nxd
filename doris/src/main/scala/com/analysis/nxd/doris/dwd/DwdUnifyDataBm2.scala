...
object DwdUnifyDataBm2 {
  ...
  def runUserData(startTimeP: String, endTimeP: String, isDeleteData: Boolean, conn: Connection): Unit = {
    ...
    JdbcUtils.execute(conn, "sql_dwd_bm2_users", sql_dwd_bm2_users)

    val end = System.currentTimeMillis()
    logger.info("BM2 user 数据归一累计耗时(毫秒):" + (end - start))
  }
  ...
  def runData(startTimeP: String, endTimeP: String, isDeleteData: Boolean, conn: Connection): Unit = {
    ...
    val sql_dwd_bm2_transactions =
      s"""
         ...
         |from
         |( select * from ods_bm2_transactions where (created_at>='$startTime' and  created_at<='$endTime') )t
         |join  (select  *  from doris_dt.dwd_users where  site_code='BM2')  t_u  on t.site_code=t_u.site_code  and   t.user_id=t_u.id
         |left join  (select * from dwd_transaction_types ) t_t   on  t.type_id=t_t.type_code and   t.site_code=t_t.site_code
         |left join  (select distinct  id,series_id,name_cn from ods_bm2_lotteries ) t_l  on  t.lottery_id=t_l.id
         |left join  (select distinct  id,name from ods_bm2_series ) t_s  on  t_l.series_id=t_s.id
         |left join  (select distinct  id,name from ods_bm2_series_ways ) t_s_w  on  t.way_id=t_s_w.id
         |left join (select distinct site_code , lottery_id, issue ,end_time2 from  ods_bm2_issues where  (end_time2>=date_sub('$startTime',3) and  end_time2<=date_add('$endTime',5) ) ) t_i  on  t.issue=t_i.issue and  t.lottery_id=t_i.lottery_id
         |""".stripMargin

    val sql_dwd_bm2_user_logins =
      s"""
         ...
         |from
         |(select *  from  ods_bm2_user_login_logs where (created_at>='$startTime' and  created_at<='$endTime') or  (updated_at>='$startTime' and  updated_at<='$endTime')) t_l
         |join  (select *  from doris_dt.dwd_users where  site_code='BM2')  t_u  on  t_l.user_id=t_u.id and  t_l.site_code=t_u.site_code
         |""".stripMargin

    val sql_dwd_bm2_user_bank =
      s"""
         ...
         |from
         |(select *  from  ods_bm2_user_bank_cards where (created_at>='$startTime' and  created_at<='$endTime')) t
         |join  (select *  from doris_dt.dwd_users where  site_code='BM2') t_u  on  t.user_id=t_u.id
         |""".stripMargin
    ...
    val sql_dwd_transaction_types =
      """
        ...
        |from
        |ods_bm2_transaction_types  b
        |left  join  dwd_transaction_types_parent p on  b.site_code=p.site_code and b.id=p.type_code
        |""".stripMargin

    val sql_del_dwd_bm2_user_logins = s"delete from  dwd_user_logins    where   site_code='BM2' and (created_at>='$startTime' and  created_at<='$endTime')"
    val sql_del_dwd_bm2_transactions = s"delete from  dwd_transactions  where   site_code='BM2' and (created_at>='$startTime' and  created_at<='$endTime')"
    val sql_del_dwd_bm2_dwd_user_bank = s"delete from  dwd_user_bank  where   site_code='BM2' and (created_at>='$startTime' and  created_at<='$endTime')"
    val start = System.currentTimeMillis()
    // 删除数据
    JdbcUtils.execute(conn, "use doris_dt", "use doris_dt")
    if (isDeleteData) {
      JdbcUtils.executeSiteDeletePartitionMonth(startTime,endTime,"",conn, "sql_del_dwd_bm2_user_logins", sql_del_dwd_bm2_user_logins)
      JdbcUtils.executeSiteDeletePartitionMonth(startTime,endTime,"",conn, "sql_del_dwd_bm2_transactions", sql_del_dwd_bm2_transactions)
      JdbcUtils.execute(conn, "sql_del_dwd_bm2_dwd_user_bank", sql_del_dwd_bm2_dwd_user_bank)
    }

    JdbcUtils.execute(conn, "sql_dwd_transaction_types", sql_dwd_transaction_types)
    JdbcUtils.execute(conn, "sql_dwd_bm2_transactions", sql_dwd_bm2_transactions)
    JdbcUtils.execute(conn, "sql_dwd_bm2_user_logins", sql_dwd_bm2_user_logins)
    JdbcUtils.execute(conn, "sql_dwd_bm2_user_bank", sql_dwd_bm2_user_bank)
//    val map: Map[String, String] = Map(
//      "sql_dwd_bm2_transactions" -> sql_dwd_bm2_transactions
//      , "sql_dwd_bm2_user_logins" -> sql_dwd_bm2_user_logins
//      , "sql_dwd_bm2_user_bank" -> sql_dwd_bm2_user_bank
//    )
//    ThreadPoolUtils.executeMap(map, conn, "doris_dt")
    val end = System.currentTimeMillis()
    logger.info("BM2 数据归一累计耗时(毫秒):" + (end - start))
  }
  ...
  def runProcessData(startTimeP: String, endTimeP: String, isDeleteData: Boolean, conn: Connection): Unit = {
    ...
    val sql_dwd_deposit =
      s"""
         ...
         |from
         |(
         |select  * from  ods_bm2_deposits where (created_at>='$startTime' and  created_at<='$endTime')
         |) t
         |join   (select *  from doris_dt.dwd_users where  site_code='BM2')  t_u  on t.site_code=t_u.site_code and  t.user_id=t_u.id
         |
         |""".stripMargin
    ...
    val sql_dwd_withdraw =
      s"""
         ...
         |from
         |(
         |select  * from  ods_bm2_withdrawals where (created_at>='$startTime' and  created_at<='$endTime')
         |) t
         |join   (select *  from  doris_dt.dwd_users where  site_code='BM2')  t_u  on t.site_code=t_u.site_code and  t.user_id=t_u.id
         |""".stripMargin
